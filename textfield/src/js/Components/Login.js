    ...
    return(
      <div className="App">
        <form class="navbar-form" id="login" onSubmit="this.handleSubmit">
            ...
            <CampoDeTexto id="email" Value={this.state.email} Names={['email']} PlaceHolders={['Ingresa tu correo']} Types={['email']}  Patterns="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" onChange="this.handleChange"/>
            <TextoExplicativo Texto={['Contraseña']}/>
            <CampoDeTexto id="password" Value={this.state.password} Names={['password']} PlaceHolders={['Ingresa tu contraseña']} Types={['password']}  Patterns="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" onChange="this.handleChange"/>
